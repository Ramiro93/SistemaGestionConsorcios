...
namespace PW3_TP.Utilities
{
    public class ComprobantesUtility
    {
        /// <summary>
        ...
        public static string Guardar(HttpPostedFileBase archivoSubido, string nombreSignificativo)
        {
            ...
            //ejemplo: /Media/Comprobantes/
            //la carpeta (con path relativo) donde se guardan las imagenes se obtiene del web.config
            string carpetaComprobantes = System.Configuration.ConfigurationManager.AppSettings["CarpetaComprobantes"];

            if (string.IsNullOrEmpty(carpetaComprobantes))
            {
                throw new Exception("En el archivo web.config debe agregar dentro de <appSettings> el elemento <add key=\"CarpetaComprobantes\" value=\"/Comprobantes/\" />");
            }
            ...
            carpetaComprobantes = string.Format("/{0}/", carpetaComprobantes.TrimStart('/').TrimEnd('/'));

            //Server.MapPath antepone a un string la ruta fisica donde actualmente esta corriendo la aplicacion (ej. c:\inetpub\misitio\)
            string pathDestino = System.Web.Hosting.HostingEnvironment.MapPath("~") + carpetaComprobantes;
            ...
            return string.Concat(carpetaComprobantes, nombreArchivoFinal);
        }
